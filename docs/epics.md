...
| E9 | Delivery hardening: backoff, EXHAUSTED, requeue, republish | payments, api | E6, E7 | M3 | ✅ 2026-09-04 — run #152 `33903041867` (S1 `OutboxExhaustionIT` 1/1, S2 `OutboxRequeueIT` 6/6 + `OutboxAdminRotationIT` 2/2 Q11 403 leg, S3 `OutboxRepublishIT` 10/10 + `OutboxRepublishRotationIT` 3/3, S4 `OutboxRepublishIT.republish_re_run_produces_identical_new_ids` deterministic `{original}-r{n}` dedupe, S5 `docs/runbooks/dlq-inspection.md`, S6 `design.md` §13 M3✅ + `CHANGELOG` 1.0.3 + TD-26 annotations) — matrix evidenced (`tasks/delivery-hardening-e9-spec.md` §10) — **M3 ✅** |
