...
| E16 | Operational hygiene: Alertmanager, PITR v2, honest k6, limiter posture, dependabot | deploy, docs, api | E12, E15 | post-1.0.0 | ◐ |
| (M5) | Stretch batch (renamed from the former "E16"; owned by the M5 scoping package — `tasks/m5-scoping.md`): card Strategy · k6 gate · Redis cache · webhook reprocessing | payments, api | E8 (card: E1, E2) | M5 | ☐ (in scoping) |
...
### E16 — Operational hygiene (post-1.0.0, in progress)
Alertmanager with a logging-stub receiver (amtool-validated in CI), PITR v2 with off-disk WAL,
an honest k6 run (spine ON + default limiter limits) published beside the 414 rps baseline,
the limiter posture closed either way, dependabot for early-warning dep PRs. **Fence:** M5
territory (card/Redis/k6-gate/reprocess) is OUT by design (design.md:640 — the M5 package is
being prepared channel-side).
