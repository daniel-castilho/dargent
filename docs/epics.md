...
| E8 | Refunds: partial/total, fee reversal, balance drain | payments, ledger, api | E4, E7 | M3 | ✅ 2026-09-03 — Block 1 (S1 refund use case, S2 endpoint, S3 ledger `refund.created` golden vector `RefundFlowIT`, HEAD `8bad5e8`/CI `33827336501`) → Block 2 (S6: `RefundBalanceGuardIT` 2/2 + `RefundRaceIT` 2/2 for scenarios 12 & 23, DB-arbitrated drain, `refund_skipped_balance` audit + skip-audit actor fix; S7: auditor refund legs (c)/(d) + connection-leak fix `JournalCoverageAuditorIT` 6/6) — full reactor `mvn -B verify` **BUILD SUCCESS** + `check-boundaries.sh` OK — matrix evidenced (`tasks/refunds-e8-spec.md` §10) — **M3 remains ◐** (E9 delivery hardening pending) |
