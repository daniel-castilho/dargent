...
> **⚠️ Operational limitation (ratified E9 §4 / §6.4):** Republish mints new events that re-notify already-notified consumers. The ledger consumer dedupes by `eventId` (scenario 20), and the **notifications consumer also dedupes by `eventId` (ON CONFLICT)** — re-runs of the same republish are idempotent at both consumers. However, replay of first trip re-notifies (new UUID) — a ratified limitation.
