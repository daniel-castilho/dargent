...
3. **Truth-correction commit (docs-only):** replace `docs/epics.md` with the corrected ledger from the E3R docs
   ...
   prepend the VOID banner to `tasks/e4-webhook-intake/e4-acceptance-matrix.md`. Push; confirm the run goes green; cite its id.
...
### Actions
1. Create `tasks/e3r-create-webhook-remediation/e3r-acceptance-matrix.md` (register ID → implementation → CI test → run id); rewrite
   `tasks/e3-create-payment/e3-acceptance-matrix.md` (prior evidence voided, superseded-by-E3R where applicable); **rebuild**
   `tasks/e4-webhook-intake/e4-acceptance-matrix.md` from scratch — the `97882494` version cited non-existent tests and is void
...
### Verify
```bash
grep -n "pending" tasks/e3r-create-webhook-remediation/e3r-acceptance-matrix.md tasks/e3-create-payment/e3-acceptance-matrix.md tasks/e4-webhook-intake/e4-acceptance-matrix.md  # expect: none
