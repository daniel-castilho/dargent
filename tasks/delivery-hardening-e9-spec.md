...
3. `OutboxRepublishIT` — window replay mints bounded new rows with salted ids; originals SENT;
   ...
   ledger+notifications), window bound enforced (400 >30d), empty window → 200/0.
4. Scenario 20 ledger leg: republished `payment.confirmed` consumed twice-equivalently → journal
   count unchanged, balances unchanged (extends the RefundFlow harness pattern).
