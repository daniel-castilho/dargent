...
- **Open defect TD-13 (spec §2):** the R7 citation layer (e3r matrix, ledger flip rows, correction note)
  ...
  fixes it. The verified pair table is in `tasks/e3r-create-webhook-remediation/e3r-block1-verification.md`.
...
1. `tasks/e6-outbox-messaging/outbox-messaging-e6-spec.md` — §4.1 env contract, §5 exact contracts, §5.7 BoE (defaults are
   DERIVED — do not tune by taste), §7 tests, §9 DoD.
2. `tasks/e6-outbox-messaging/outbox-messaging-e6-implementation-sequence.md` — S0…S7 order, commit messages, global rules,
   failure playbooks.
3. `tasks/e6-outbox-messaging/outbox-messaging-e6-backlog.md` — acceptance anchors per item.
4. `tasks/e3r-create-webhook-remediation/create-webhook-remediation-e3r-spec.md` §2 + `tasks/e3r-create-webhook-remediation/e3r-block1-verification.md` — the governance
