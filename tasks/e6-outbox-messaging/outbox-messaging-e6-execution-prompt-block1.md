...
- **TD-13 is open** (e3r spec §2): the R7 citation layer carries register misquotes (TD-9's off-by-one
  ...
  `tasks/e3r-create-webhook-remediation/e3r-block1-verification.md`. Your FIRST commit fixes this — E6 never builds on lying docs.
...
1. `tasks/e6-outbox-messaging/outbox-messaging-e6-spec.md` — §2 current state, §4.1 env contract, §5 exact contracts, §5.7 BoE.
2. `tasks/e6-outbox-messaging/outbox-messaging-e6-implementation-sequence.md` — S0…S7, global rules, failure playbooks.
3. `tasks/e6-outbox-messaging/outbox-messaging-e6-backlog.md` — acceptance anchors.
4. `tasks/e3r-create-webhook-remediation/create-webhook-remediation-e3r-spec.md` §2 + `tasks/e3r-create-webhook-remediation/e3r-block1-verification.md` — inherited
...
1. `tasks/e3r-create-webhook-remediation/e3r-acceptance-matrix.md`: rewrite the register-traceability table from the **committed e3r spec
