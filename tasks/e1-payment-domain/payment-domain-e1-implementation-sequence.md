...
2. Fill `tasks/e1-payment-domain/e1-acceptance-matrix.md` (evidence: run link, test class names, matrix of transition rows).
...
### Verify
```bash
grep -c pending tasks/e1-payment-domain/e1-acceptance-matrix.md   # expect 0
...
| 2026-08-28 | S8 | `PaymentJpaAdapter.updateIfVersionMatches` uses an explicit **JPQL conditional UPDATE** (`SET … version = :expected+1 WHERE txid = :txid AND version = :expected`, zero rows → `false`) instead of a managed-entity flush catching `OptimisticLockException`. A failed flush marks the transaction rollback-only, so the loser's commit throws `UnexpectedRollbackException` even when the exception is caught — the mandatory lost-race semantics (AGENTS.md §3.2, spec §6: "adapter NEVER throws on a lost race") cannot be met by catch-then-return. The DB arbitrates; `@Version` stays on the entity as the defensive second line but is no longer the arbitration mechanism the contract suite exercises. |
| 2026-08-28 | S8 | Entity `id` is **DB-generated** (`@GeneratedValue(GenerationType.UUID)`); `PaymentMapper.toEntity` no longer assigns a value. The row key is not part of the balance-carrying aggregate, and assigning `UUID.randomUUID()` collided with `@GeneratedValue` (Hibernate rejects a non-null id on `persist` as detached). App-generated UUIDv7 (D3) remains out of E1 scope; documented in `PaymentMapper` javadoc. |
