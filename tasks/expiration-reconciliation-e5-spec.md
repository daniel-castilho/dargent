...
|---|---|---|---|---|
| S0 | DEBT-1 rejecting-contract test | `PaymentTest.restore_rejects_*` (3 rejecting tests + legal round-trip; module unit tests)` | **#125 `33693408878`** | ✅ |
| S1 | V111 reconciliation columns + pending-expires index used | `V111__e5_reconciliation.sql` applied; `ExpirationSchedulerIT` + reconciler harness exercise the index | **#126 `33694469538`** | ✅ |
| S2 | expiration contract | `ExpirationSchedulerIT` | **#127 `33700561182`** | ✅ |
| S3 | reconciler confirms (sc.26) | `ReconcilerConfirmIT` | **#128 `33706674658`** | ✅ |
| S4 | resurrection exactly-once (sc.11/27) | `ReconcilerResurrectionIT` | **#129 `33707174938`** | ✅ |
| S5 | give-up window | `ReconcilerGiveUpIT` (renamed from the spec's `ReconcilerGiveUpIt` — failsafe include is case-sensitive `**/*IT.java`) | **#130 `33709904795`** | ✅ |
| S6 | scenarios 9/10 legs | `ReconcilerConsistencyIT` | **#131 `33710432248`** | ✅ |
| S7 | DEBT-4 coverage auditor | `JournalCoverageAuditorIT` | **#132 `33711320405`** | ✅ |
...
> green (run #130 `33709904795`). Both are doc-level corrections; no spec behavior change.
