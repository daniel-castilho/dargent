...
class ReconcilerConfirmIT {
    ...
    void psp_paid_payment_is_confirmed_with_late_false_outbox_and_audit() {
        ...
        assertThat(jdbc.sql(
                                "select merchant_id from payments.audit_log where command_name='confirm_from_reconciliation' and aggregate_id=:t")
                        .param("t", txid)
                        .query(UUID.class)
                        .single())
                .isEqualTo(MERCHANT);
    ...
    void second_reconciliation_run_on_confirmed_payment_is_an_idempotent_no_op() {
        ...
        assertThat(jdbc.sql(
                        ...
                        .single())
                .isEqualTo(1);
        assertThat(jdbc.sql("select status from payments.payments where txid=:t")
                        .param("t", txid)
                        .query(String.class)
                        .single())
                .isEqualTo("CONFIRMED");
