...
class ApiKeyAuthenticationFilterTest {
    ...
    void missing_authorization_header_returns_401() throws Exception {
        ...
        verify(writer).write(request, response, ErrorCode.UNAUTHORIZED, "Missing or invalid Authorization header", (Map<
                        String, String>)
                null);
    }
    ...
    void valid_key_sets_principal_in_security_context() throws Exception {
        ...
        final org.springframework.security.authentication.UsernamePasswordAuthenticationToken[] capturedAuth =
        ...
        Filter observing = (req, res, chain) ->
                capturedAuth[0] = (org.springframework.security.authentication.UsernamePasswordAuthenticationToken)
                        SecurityContextHolder.getContext().getAuthentication();
        MockFilterChain chain = new MockFilterChain(testServlet, observing);

        when(repository.findByPrefix(prefix))
                .thenReturn(Optional.of(new ApiKeyRecord(keyId, merchantId, prefix, hash, null)));

        filter.doFilter(request, response, chain);

        assertThat(capturedAuth[0]).isNotNull();
        assertThat(capturedAuth[0].getPrincipal()).isInstanceOf(ApiKeyPrincipal.class);
        assertThat(((ApiKeyPrincipal) capturedAuth[0].getPrincipal()).merchantId())
    ...
    void wrong_hash_returns_401() throws Exception {
        ...
        when(repository.findByPrefix(prefix))
                .thenReturn(
                        Optional.of(new ApiKeyRecord(UUID.randomUUID(), UUID.randomUUID(), prefix, wrongHash, null)));
