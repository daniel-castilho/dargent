...
| DEBT-4 | Money state can dangle: a payment confirmed yet never journaled (or a POSTED journal event with no confirmed payment) was previously undetectable. Add a coverage auditor. Extended in E8 S7 with refund legs (c) refund row with no POSTED `refund.created` and (d) POSTED `refund.created` with no refund row. | — | E5 (paid — `JournalCoverageAuditor` + `JournalCoverageAuditorIT`, `470e10c`/`33711320405`); refund legs + connection-leak fix in E8 S7 |
| DEBT-5 | `JournalCoverageAuditor.runOnce()` used `JdbcClient.query(...).stream().collect(...)` — a lazy `ResultSet`-backed stream never closed, leaking one DB connection per query per scan until the Hikari pool exhausted under IT load (`CannotGetJdbcConnectionException`). Fixed by materializing all four queries with `.list()` (terminal op closes the stream). **Balanced-entry barrier** `8dae52d` (JournalEntry ctor validation + DB trigger) prevents recurrence. | — | E8 Block 2 (paid — S7, `JournalCoverageAuditor` refactor + `JournalCoverageAuditorIT` 6/6) |
| DEBT-6 | Connection leak in `JournalCoverageAuditor` — lazy stream `.collect()` leaked one DB connection per query per scan. Fixed in `7993f1f` (materialize `.list()`); verified by `JournalCoverageAuditorIT` 6/6 under E8 Block 2 load. | — | E8 Block 2 (paid — `JournalCoverageAuditor` refactor + `JournalCoverageAuditorIT` 6/6) |
| DEBT-7 | ~~`postJournal`/`postJournalWithoutBalances` duplication in the ledger store adapters~~ **RESOLVED 2026-09-08 (E15 S6, Path A)**: consolidated behind one guarded implementation `postJournalCore(entry, updateBalances)` — payment path posts with balance upserts, refund path posts with `false` (the E8 conditional-drain owns the balance writes atomically). Proof: full suite green; coverage floors PASS (ledger LINE 86.9 measured post-refactor vs 87.3 baseline same-tree — the −0.4pt is the removed duplicate lines, floor 75 untouched); E8/E9 guarantees re-evidenced: RefundRaceIT 2/2, RefundBalanceGuardIT 2/2, RefundFlowIT 2/2, Scenario20NoDoubleJournalIT 1/1, JournalCoverageAuditorIT 6/6, JournalBalanceDbBarrierIT 4/4 — all green post-consolidation. | — | E15 S6 (RESOLVED) |
| DEBT-8 | ~~No rate limit or request body cap on the public webhook route (`POST /webhooks/psp`)~~ **RESOLVED 2026-09-08 (E15 S1)**: in-app `WebhookAbuseControlFilter` — per-IP token-bucket rate limit (429) + body cap (413) on the webhook route only, control order carved by ITs (`WebhookRateLimitIT`, `WebhookBodyCapIT`); defaults generous (100 burst / 0.5 rps refill / 64 KiB cap), env-tunable (`DARGENT_WEBHOOK_RATE_LIMIT_*`, `DARGENT_WEBHOOK_BODY_CAP_BYTES`). NGINX edge rate-limiting remains defense-in-depth for a future public deployment. | — | E15 S1 (RESOLVED) |
