...
| DEBT-8 | No rate limit or request body cap on the public webhook route (`POST /webhooks/psp`) — a webhook flood is bounded only by payload persistence, not by a request-rate control (E13 S5 threat model, surface 1, DoS row). Disposition: accepted for v1 (route is PSP-only in the prod topology; NGINX edge rate-limiting **and request body cap** when a public deployment exists). | — | post-v1.0.0 candidate |
