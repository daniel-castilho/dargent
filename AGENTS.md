...
| DEBT-5 | `JournalCoverageAuditor.runOnce()` used `JdbcClient.query(...).stream().collect(...)` — a lazy `ResultSet`-backed stream never closed, leaking one DB connection per query per scan until the Hikari pool exhausted under IT load (`CannotGetJdbcConnectionException`). Fixed by materializing all four queries with `.list()` (terminal op closes the stream). | — | E8 Block 2 (paid — S7, `JournalCoverageAuditor` refactor + `JournalCoverageAuditorIT` 6/6) |
| DEBT-6 | Connection leak in `JournalCoverageAuditor` (reverted) — lazy stream `.collect()` leaked one DB connection per query per scan. Fixed in `03d7967` (revert) + `470e10c` (materialize `.list()`). | — | E8 Block 2 (paid — `JournalCoverageAuditor` refactor + `JournalCoverageAuditorIT` 6/6) |
