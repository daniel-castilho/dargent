...
class EventIngestionUseCaseTest {
    ...
    void processes_confirmed_payment_and_acks() {
        ...
        verify(jdbc)
                .sql(org.mockito.ArgumentMatchers.argThat(s -> s.toString().contains("UPDATE ledger.events")
                        && s.toString().contains("status = 'POSTED'")));
    }
    ...
    void duplicate_received_resumes_and_posts_once() {
        ...
        store.insertedEvents.put(
                eventId,
                new FakeLedgerStore.EventRecord(
                        eventId,
                        "payment.confirmed",
                        "txid-123",
    ...
    void duplicate_ignored_ack_skips() {
        ...
        store.insertedEvents.put(
                ...
                new FakeLedgerStore.EventRecord(
                        ...
                        raw,
                        "IGNORED",
                        "Non-confirmed type: payment.created"));
    ...
    static class FakeLedgerStore implements LedgerStore {
        ...
        public Optional<String> findEventStatus(UUID eventId) {
            return Optional.ofNullable(insertedEvents.get(eventId)).map(EventRecord::status);
        }
        ...
        public void postJournal(JournalEntry entry) {
            ...
            insertedEvents.computeIfPresent(
                    entry.eventId(),
                    (id, record) -> new EventRecord(
                            record.eventId(),
                            record.type(),
                            record.txid(),
                            record.merchantId(),
