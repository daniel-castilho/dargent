...
class SqsEventConsumerTest {
    ...
    void ingestion_returns_false_message_not_acked_stays_for_redelivery() {
        ...
        Message poisonMsg = Message.builder()
                ...
                .body(
                        "{\"eventId\":\"123e4567-e89b-12d3-a456-426614174000\",\"type\":\"payment.confirmed\",\"version\":1,\"aggregateId\":\"txid\",\"merchantId\":\"11111111-1111-1111-1111-111111111111\",\"occurredAt\":\"2026-08-30T12:00:00Z\",\"payload\":\"not valid json\"}")
                .build();

        when(sqsClient.receiveMessage(any(ReceiveMessageRequest.class)))
                .thenReturn(ReceiveMessageResponse.builder()
                        .messages(List.of(poisonMsg))
                        .build());
        when(ingestion.processMessage(anyString())).thenReturn(false); // poison -> nack

        SqsEventConsumer consumer = new SqsEventConsumer(sqsClient, "http://queue", 10, 1000, ingestion);
    ...
    void ingestion_returns_true_message_acked_and_deleted() {
        ...
        Message validMsg = Message.builder()
                ...
                .body(
                        "{\"eventId\":\"123e4567-e89b-12d3-a456-426614174000\",\"type\":\"payment.confirmed\",\"version\":1,\"aggregateId\":\"txid\",\"merchantId\":\"11111111-1111-1111-1111-111111111111\",\"occurredAt\":\"2026-08-30T12:00:00Z\",\"payload\":{\"txid\":\"txid\",\"merchantId\":\"11111111-1111-1111-1111-111111111111\",\"amount\":10000,\"fee\":100,\"net\":9900,\"late\":false}}")
                .build();

        when(sqsClient.receiveMessage(any(ReceiveMessageRequest.class)))
                .thenReturn(ReceiveMessageResponse.builder()
                        .messages(List.of(validMsg))
                        .build());
        ...
        verify(sqsClient)
                .deleteMessageBatch(
                        argThat((DeleteMessageBatchRequest req) -> req.entries().size() == 1
    ...
    void mixed_poison_and_valid_only_valid_acked() {
        ...
        when(sqsClient.receiveMessage(any(ReceiveMessageRequest.class)))
                .thenReturn(ReceiveMessageResponse.builder()
                        .messages(List.of(poisonMsg, validMsg))
                        .build());
        ...
        assertThat(processed).isEqualTo(1);
        verify(sqsClient)
                .deleteMessageBatch(argThat((DeleteMessageBatchRequest req) ->
                        req.entries().size() == 1 && req.entries().get(0).id().equals("valid-1")));
    }
}
